...
if ($is_edit) {
  ...
  try {
    $stmt = $pdo->prepare("SELECT * FROM recursos_multimedia WHERE clase_id = ? AND schema_role = 'primary' ORDER BY sort_order ASC, id ASC LIMIT 1");
    $stmt->execute([$id]);
        ...
        if ($exists > 0) {
          ...
          if (!empty($imagen_portada)) {
            try {
              ...
              $sel = $pdo->prepare("SELECT id FROM recursos_multimedia WHERE clase_id = ? AND schema_role = 'primary' ORDER BY sort_order ASC, id ASC LIMIT 1");
              $sel->execute([$id]);
              ...
              if ($rm_id > 0) {
                ...
                $upd->execute([$imagen_portada, ($media_titulo !== '' ? $media_titulo : $nombre), ($media_descripcion !== '' ? $media_descripcion : $resumen), $role, ($media_mime_type !== '' ? $media_mime_type : 'image/webp'), $media_width, $media_height, $upload_dt, ($media_in_language !== '' ? $media_in_language : 'es-CO'), ($media_creator_name !== '' ? $media_creator_name : 'Clase de Ciencia'), $rm_id]);
              } else {
                $insRm = $pdo->prepare("INSERT INTO recursos_multimedia (clase_id, tipo, url, titulo, descripcion, sort_order, schema_role, mime_type, width, height, upload_date, in_language, creator_name) VALUES (?, 'imagen', ?, ?, ?, 0, ?, ?, ?, ?, ?, ?, ?)");
                $insRm->execute([$id, $imagen_portada, ($media_titulo !== '' ? $media_titulo : $nombre), ($media_descripcion !== '' ? $media_descripcion : $resumen), $role, ($media_mime_type !== '' ? $media_mime_type : 'image/webp'), $media_width, $media_height, $upload_dt, ($media_in_language !== '' ? $media_in_language : 'es-CO'), ($media_creator_name !== '' ? $media_creator_name : 'Clase de Ciencia')]);
              }
            } catch (PDOException $e) {
              // Fallback esquema base
              try {
                $sel = $pdo->prepare("SELECT id FROM recursos_multimedia WHERE clase_id = ? AND tipo = 'imagen' ORDER BY sort_order ASC, id ASC LIMIT 1");
                $sel->execute([$id]);
                $rm_id = (int)$sel->fetchColumn();
                if ($rm_id > 0) {
                  $upd = $pdo->prepare("UPDATE recursos_multimedia SET url=?, titulo=?, descripcion=?, sort_order=0 WHERE id=?");
                  $upd->execute([$imagen_portada, ($media_titulo !== '' ? $media_titulo : $nombre), ($media_descripcion !== '' ? $media_descripcion : $resumen), $rm_id]);
                } else {
                  $insRm = $pdo->prepare("INSERT INTO recursos_multimedia (clase_id, tipo, url, titulo, descripcion, sort_order) VALUES (?, 'imagen', ?, ?, ?, 0)");
                  $insRm->execute([$id, $imagen_portada, ($media_titulo !== '' ? $media_titulo : $nombre), ($media_descripcion !== '' ? $media_descripcion : $resumen)]);
                }
              } catch (PDOException $e2) {
                echo '<script>console.log("⚠️ [ClasesEdit] No se pudo guardar metadata multimedia:",' . json_encode($e2->getMessage()) . ');</script>';
              }
            }
          }
  ...
  (function initIaClassBuilder() {
    ...
    function buildIaPrompt(userMessage) {
      ...
      return [
        ...
        '{"nombre":"","slug":"","ciclo":1,"grados":[6,7],"dificultad":"facil|media|dificil","duracion_minutos":60,"resumen":"","objetivo_aprendizaje":"","contenido_html":"","seo_title":"","seo_description":"","seguridad":{"edad_min":0,"edad_max":0,"notas":""},"tags":["tag1","tag2"],"autor":"","status":"draft|published","areas_nombres":["Física"],"competencias_sugeridas":["texto breve"],"plantilla_estructura":{"secciones":["Introduccion","Para que sirve","Modelo del fenomeno","Componentes y rol academico","Funcionamiento","Variables y relaciones clave","Actividades de analisis conceptual","Evaluacion formativa","Glosario basico"]}}',
        'Reglas obligatorias para contenido_html:',
        '- Debe usar esta secuencia de secciones con h2 en este orden exacto:',
        '  1) Introduccion',
        '  2) Para que sirve',
        '  3) Modelo del fenomeno',
        '  4) Componentes y rol academico',
        '  5) Funcionamiento',
        '  6) Variables y relaciones clave',
        '  7) Actividades de analisis conceptual',
        '  8) Evaluacion formativa',
        '  9) Glosario basico',
        '- Usar solo etiquetas: h2, p, ul, ol, li, strong, em.',
        '- Escribir en tono escolar, claro, cientifico y seguro.',
        '- Ajustar profundidad segun ciclo/dificultad: menos tecnicismo en ciclos bajos, mayor rigor en ciclos altos.',
        '- No inventar productos inexistentes ni instrucciones peligrosas.',
        '- slug debe iniciar por clase-.',
        '- Si falta informacion, completar con supuestos razonables y consistentes con el contexto.',
  ...
  (function initIaContentBuilder() {
    ...
    function buildPrompt(userText) {
      ...
      return [
        ...
        'Schema obligatorio:',
        '{"contenido_html":"","resumen":"","objetivo_aprendizaje":"","notas_autor":"","plantilla_validada":true,"secciones_detectadas":["Introduccion","Para que sirve","Modelo del fenomeno","Componentes y rol academico","Funcionamiento","Variables y relaciones clave","Actividades de analisis conceptual","Evaluacion formativa","Glosario basico"]}',
        'Reglas:',
        ...
        '  8) Evaluacion formativa',
        '  9) Glosario basico',
        '- Incluir enfoque didactico, lenguaje claro, seguridad y evaluacion formativa.',
