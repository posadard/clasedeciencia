    ...
    if (empty($errores)) {
      try {
        ...
        if ($is_edit) {
          ...
          $stmt->execute([$nombre_comun, $sku, $categoria_id, $advertencias_seguridad_json, $unidad, $descripcion_html, ($foto_url !== '' ? $foto_url : null), $id]);
        } else {
          $sql = "INSERT INTO kit_items (nombre_comun, sku, categoria_id, advertencias_seguridad, unidad, descripcion_html, foto_url) VALUES (?, ?, ?, ?, ?, ?, ?)";
          $stmt = $pdo->prepare($sql);
          $stmt->execute([$nombre_comun, $sku, $categoria_id, $advertencias_seguridad_json, $unidad, $descripcion_html, ($foto_url !== '' ? $foto_url : null)]);
