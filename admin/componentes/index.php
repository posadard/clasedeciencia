...
<div class="page-header">
  ...
  <script>
    console.log('✅ [Admin] Componentes index cargado');
    ...
    console.log('🔍 [Admin] Total componentes:', <?= count($materiales) ?>);
  </script>
</div>
...
<div class="filters-bar">
  <form method="GET" class="filters-form">
    ...
    <div class="filter-group search-group">
      ...
      <input type="text" name="search" id="search" value="<?= htmlspecialchars($search, ENT_QUOTES, 'UTF-8') ?>" placeholder="Nombre común o advertencias..." />
      <button type="submit" class="btn btn-sm">🔍 Buscar</button>
      <?php if ($category || $search): ?>
        <a href="/admin/componentes/index.php" class="btn btn-sm btn-secondary">Limpiar</a>
      <?php endif; ?>
    </div>
  </form>
</div>
...
<div class="card" style="margin-bottom:1rem;display:flex;justify-content:space-between;align-items:center;">
  ...
  <a href="/admin/componentes/edit.php" class="btn">+ Nuevo Componente</a>
</div>

<?php if (empty($materiales)): ?>
  <div class="empty-state">
    <p>No hay componentes.</p>
    <a href="/admin/componentes/edit.php" class="btn btn-primary">Crear Componente</a>
  </div>
<?php else: ?>
  <table class="data-table">
    ...
    <?php foreach ($materiales as $m): ?>
      <tr>
        <td><?= (int)$m['id'] ?></td>
        <td><strong><?= htmlspecialchars($m['common_name'], ENT_QUOTES, 'UTF-8') ?></strong></td>
        <td><span class="badge"><?= htmlspecialchars(($m['category_name'] ?? '-'), ENT_QUOTES, 'UTF-8') ?></span></td>
        <td><?= htmlspecialchars($m['slug'], ENT_QUOTES, 'UTF-8') ?></td>
        <td class="actions">
          <a href="/admin/componentes/edit.php?id=<?= (int)$m['id'] ?>" class="btn btn-sm btn-edit">Editar</a>
        </td>
      </tr>
    <?php endforeach; ?>
    </tbody>
