  ...
  <form method="GET" class="filters-form">
    <div class="filter-group">
      ...
      <select name="ciclo" id="ciclo" onchange="this.form.submit()">
        ...
        <?php 
        $ciclos_proy = cdc_get_ciclos($pdo, true);
        foreach ($ciclos_proy as $cp): 
            $sel = ($ciclo == $cp['numero']) ? 'selected' : '';
        ?>
        <option value="<?= (int)$cp['numero'] ?>" <?= $sel ?>><?= h($cp['numero']) ?> - <?= h($cp['nombre']) ?></option>
        <?php endforeach; ?>
      </select>
    </div>
    <div class="filter-group">
  ...
  <table class="data-table">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td class="actions">
          <a href="/clase.php?slug=<?= htmlspecialchars($p['slug'], ENT_QUOTES, 'UTF-8') ?>" target="_blank" class="btn btn-secondary action-btn">Ver</a>
